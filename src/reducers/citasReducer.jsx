...
export const citasReducer = (state, action) =>{
    ...
    if(action.type === 'RESERVAR_CITA'){
        const listaDeCitas = state.listaDeCitas.map(cita => cita.id === action.id ? {...cita,available:false}:
            cita)
    ...
    if(action.type === 'CANCELAR_CITA'){
        const listaDeCitas = state.listaDeCitas.map(cita => cita.id === action.id ? {...cita,available:true}:
            cita)
